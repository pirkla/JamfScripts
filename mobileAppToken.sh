...
ids=$(curl -H "Content-Type: application/xml" -ksu "$jssUser":"$jssPass" "${jssURL}/mobiledeviceapplications" -X GET | xpath "//id[not(ancestor::site)]" 2> /dev/null | sed s/'<id>'//g | sed s/'<\/id>'/','/g)
...
do
    ...
    vpp=$(curl -H "Accept : text/xml" -H "Content-Type: text/xml" -ksu "$jssUser":"$jssPass" "$jssURL/mobiledeviceapplications/id/$curID/subset/vpp" -X GET)
	...
	if [ "$token" == "$oldToken" ]; then
		    update=$(curl -H "Accept : text/xml" -H "Content-Type: text/xml" -ksu "$jssUser":"$jssPass" "$jssURL/mobiledeviceapplications/id/$curID" -X PUT -d  "<mobile_device_application>
                <vpp>
